...
def feature_4(s, e, use_dontdothat=True):
    ...
    if s is None:
        ...
        repeat_one_hot = [0.] * 11
        dontdothat = [0.] * len(e.action_space())
        ...
        if not master_acts:
            ...
            last_act_onehot[e.action_space().index(summary_acts[-1])] = 1.
        ...
        if use_dontdothat:
            ...
            rez = np.array(flatten + last_act_onehot + repeat_one_hot + dontdothat)
        else:
            rez = np.array(flatten + last_act_onehot + repeat_one_hot)
